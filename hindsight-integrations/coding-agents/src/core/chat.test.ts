...
describe("retainLiveSession — incremental write-back", () => {
  ...
  it("buffers a failed append and replays it, instead of re-sending the whole transcript", async () => {
    ...
    const tailMessages = (tail[0] as string).split("\n").map((l) => JSON.parse(l));
    expect(tailMessages.map((message) => message.sequence)).toEqual([5, 6]);
    expect(tailMessages.map((message) => message.content[0].text)).toEqual(["turn 4", "turn 5"]);
    expect(tailMessages.every((message) => message.type === CANONICAL_MESSAGE_TYPE)).toBe(true);
